define(function (require) {
    ...
    app.controller('customerordersCtrl', ['$scope', '$state', '$http',  function ($scope, $state, $http) {
        ...
        if (typeof app.configs.listenLock.customerorders == 'function') {
            document.removeEventListener("plusscrollbottom", app.configs.listenLock.customerorders, false);
        }
        ...
        $scope.customerordersinit = function () {
            ...
            app.configs.listenLock.customerorders = function (e) {
                //如是其它页则删除此监听
                if ($state.current.name != 'customerorders') {
                    document.removeEventListener("plusscrollbottom", app.configs.listenLock.customerorders, false);
                    return false;
                }
                ...
                if (!state) {
                    ...
                    $http({ method: 'POST', url: app.configs.orders.orderlist, data: $scope.orders }).
            ...
            app.configs.listenLock.customerorders();
            document.removeEventListener("plusscrollbottom", app.configs.listenLock.customerorders, false);
            document.addEventListener("plusscrollbottom", app.configs.listenLock.customerorders, false);
        };
